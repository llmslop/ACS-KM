...
public class VRPTW_ACS implements Runnable {
    ...
    static void update_statistics(VRPTW instance) {
		...
		if (ls_flag) {
        	...
        	if(Ants.acs_km_flag)
        	{
        		...
        		a = km_match_iterated(a,instance);
        	}
   ...
   //at every 5 iterations save the value of the longest cost of the solution/tour given by the best so far ant
	...
	//new local_search ways
	//take whole tour to calc (0,v1,v2,....,0)
	static int calc_tour_dist( ArrayList<Integer> vec,VRPTW vrp) {
		int ans = 0;
		ArrayList<Request> reqList = vrp.getRequests();
		double distance, arrivalTime, beginService, currentQuantity;
		double currentTime = 0.0;
		...
		{
			int prevCity = vec.get(i-1);
			int currentCity = vec.get(i);
			distance = VRPTW.instance.distance[prevCity+1][currentCity+1];
			currentQuantity = currentQuantity + reqList.get(currentCity+1).getDemand();
			if (currentQuantity > vrp.getCapacity())
			{
		...
		ans = (new Double(totaldist * 300.0)).intValue();
		
		return -ans;
	}
	
	static Ant km_match_iterated(Ant a,VRPTW instance) {
		...
		Ant improvedAnt = new Ant();
		improvedAnt.tours = new ArrayList<ArrayList<Integer>>();
		improvedAnt.tour_lengths = new ArrayList<Double>();
		improvedAnt.beginService = new double[VRPTW.n + 1];
		improvedAnt.currentTime = new ArrayList<Double>();
		improvedAnt.currentQuantity = new ArrayList<Double>();
	    ...
	    improvedAnt.earliestTime = new ArrayList<ArrayList<Double>>(improvedAnt.usedVehicles);
	    improvedAnt.latestTime = new ArrayList<ArrayList<Double>>(improvedAnt.usedVehicles);
		...
		KMmatch kmsolver = new KMmatch();
		kmsolver.resize( improvedAnt.usedVehicles );
		...
		while (foundImprovement) {
    		...
    		int max_diff = 0;
    		int [] split_ways = new int [improvedAnt.usedVehicles+1];
    		int [] match = new int [improvedAnt.usedVehicles+1];
			
    		int [] best_split = new int [improvedAnt.usedVehicles+1];
    		
    		int lb = Math.min(improvedAnt.usedVehicles, lastCommitedIndexes.size());
    		for(int i=0;i<lb;i++ )
    		{
    			split_ways[i] = lastCommitedIndexes.get(i);
    		}
    		
    		for(int i=lb;i<improvedAnt.usedVehicles;i++)
    		{
    			split_ways[i] = 0;
    		}
    		
    		
    		//calculate distance by commited id
    		for(int i=0;i<improvedAnt.usedVehicles;i++)
    		{
    			for(int j=0;j<improvedAnt.usedVehicles;j++)
    			{
    				ArrayList<Integer> ctour= new ArrayList<Integer>();
    				//ctour.add(-1);
    				
    				int left_end = split_ways[i]+1;
    				
    				//add left
    				for(int id=0;id<left_end;id++)
    				{
    					ctour.add( improvedAnt.tours.get(i).get(id) );
    				}
    				
    				int right_begin = split_ways[j]+1;
    				
    				for(int id=right_begin;id<improvedAnt.tours.get(j).size();id++)
    				{
    					ctour.add( improvedAnt.tours.get(j).get(id) );
    				}
    				
    				//negative weight
    				kmsolver.w[i][j] = calc_tour_dist(ctour,instance);
    			}
    		}
    		...
    		while(true)
    		{
        		...
        		for(int i=0;i<improvedAnt.usedVehicles;i++)
        		{
        			sum_init += kmsolver.w[i][i];
        		}
    			sum_init = -sum_init;
    			int min_weight = -kmsolver.km();
    			int delta = sum_init - min_weight;
    			if(delta > max_diff)
    			{
    				max_diff = delta;
    				for(int id=0;id<improvedAnt.usedVehicles;id++)
    				{
    					best_split[id] = split_ways[id];
    					match[id] = kmsolver.son_y[id];
    				}
    			}
    			...
    			double min_begin = 1000000000.0;
    			
    			for(int i=0;i<improvedAnt.usedVehicles;i++)
    			{
    				int j = split_ways[i]+1;
    				//stops at position x (...,x,0)
    				if( j >= improvedAnt.tours.get(i).size()-1)
    				{
    					continue;
    				}
    				int city = improvedAnt.tours.get(i).get(j);
    				if(city==-1)
    				{
    					continue;
    				}
    				
    				//select minimal begin time 
    				double cur_time = improvedAnt.beginService[city+1];
    				if( cur_time < min_begin)
    				{
    					min_begin = cur_time;
    					min_id = i;
    				}
    		...
    		if(max_diff != 0)
    		{
    			...
    			for(int j=0;j<improvedAnt.usedVehicles;j++)
    			{
    				...
    				if(i!=j)
    				{
        				...
        				int left_end = best_split[i]+1;
        				
        				//add left
        				for(int id=0;id<left_end;id++)
        				{
        					ctour.add( a.tours.get(i).get(id) );
        				}
        				
        				int right_begin = best_split[j]+1;
        				
        				for(int id=right_begin;id<a.tours.get(j).size();id++)
        				{
        					ctour.add( a.tours.get(j).get(id) );
        				}
        				
        				//improve[i] = ctour and update
        				
        				updateWholetour(improvedAnt,instance,ctour,i);
    				}
    			}
    			...
    			for(int i=0;i<improvedAnt.usedVehicles;i++)
    			{
    				new_dis += improvedAnt.tour_lengths.get(i);
    			}
    			improvedAnt.total_tour_length = new_dis;
    		}
	...
	static void updateWholetour(Ant a, VRPTW vrp, ArrayList<Integer> newTour, int tid)
	{
		int currentCity, prevCity;
		double currentTime = 0.0;
		double distance, arrivalTime, beginService = 0.0;
		...
		a.tours.get(tid).clear();
		a.tours.get(tid).add(-1);
		
		double totaldist = 0.0;
		
		for(int i=1;i<newTour.size();i++)
		{
			prevCity = newTour.get(i-1);
			currentCity = newTour.get(i);
			distance = VRPTW.instance.distance[prevCity + 1][currentCity + 1];
	    	arrivalTime = currentTime + reqList.get(prevCity + 1).getServiceTime() + distance; 
	    	beginService = Math.max(arrivalTime, reqList.get(currentCity + 1).getStartWindow());
	    	currentTime = beginService;
	    	a.beginService[currentCity + 1] = beginService;	
	    	if (beginService > reqList.get(currentCity + 1).getEndWindow()) {
	    		System.out.println("KMmatch: Unfeasible solution..");
	    	}
	    	...
	    	a.tours.get(tid).add(currentCity);
	    	
	    	totaldist += distance;
		}
		a.currentTime.set(tid,beginService);
		
		a.tour_lengths.set(tid,totaldist);
